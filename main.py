...
# Function for waiting for mouse release when reentering the main game selection screen
def wait_for_mouse_release(clock):
    waiting = True
    while waiting:
...
# Function to safely launch games
def launch_game_safely(game_function, username):
    try:
        game_function(username)
    except pygame.error as e:
        print(f"Pygame error occurred: {e}")
        pygame.quit()  # Make sure pygame is quit properly after an error
...
def main():
    ...
    while running:
        ...
        # If the start button is not pressed load the game selection menu with all its associated buttons by drawing them to the screen
            ...
            if cpm_button_clicked or pong_button_clicked:
                pass
            else:
